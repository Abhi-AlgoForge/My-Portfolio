...
function initLayout() {
    sections.forEach((sec, i) => {
        ...
        if (i === 0) {
            ...
            sec.style.transform = 'translateY(150vh)'; // Start further off-screen
            sec.style.opacity = '1';
        }
    });
...
window.addEventListener('wheel', (e) => {
    // 1. Block Global Scroll if Sector Grid or Project Details is Open
    ...
    if ((sectorOverlay && sectorOverlay.classList.contains('active')) ||
        (detailsOverlay && detailsOverlay.classList.contains('active'))) {
        // Stop propagation and allow default scroll behavior within overlays
        e.stopPropagation();
        return;
    }
    ...
    const updateUI = () => {
        ...
        sections.forEach((sec, i) => {
            ...
            if (i > currentSectionIndex) {
                sec.style.transform = 'translateY(150vh)'; // Push further down to hide overflowing elements
                sec.style.opacity = '1';
                sec.style.filter = 'none';
...
**Key Features:**
- Real-time monitoring of 100+ trading pairs across 5 exchanges
- latency compensation algorithm that adjusts order timing based on historical exchange response times
- automatic rebalancing when funds become unbalanced between exchanges
- configurable minimum profit thresholds
- comprehensive logging and Telegram alerts for executed trades.

**Limitations:**
- Arbitrage opportunities have shrunk significantly as markets mature and more participants compete for similar spreads.
- The system requires substantial capital pre-positioned on multiple exchanges (reducing capital efficiency).
- Withdrawal limits, network congestion (especially for ETH-based tokens), and occasional exchange downtime can trap funds.
- High-frequency trading on some exchanges may trigger API rate limits or account flags.
...
**Key Features:**
- Configurable position multiplier (0.1x to 5x of leader's position size)
- maximum position limits to cap exposure
- automatic stop-loss synchronization
- slippage protection that cancels copy if price moves more than X% from leader's entry
- real-time P&L dashboard comparing follower vs leader performance
- support for multiple leaders with weighted allocation.

**Limitations:**
- There is an inherent latency (100-500ms) between the leader's trade and the follower's copy, which can result in different entry prices—especially in volatile markets.
- Large followers copying a leader with limited liquidity can cause market impact.
- The system relies on the leader's continued performance; past results do not guarantee future success.
- Binance may impose restrictions on API usage patterns that resemble this activity.
- Managing leverage appropriately is critical to avoid liquidation.`
    },
    {
        category: 'trading',
        title: 'Volume Bot',
...
**Key Features:**
- Configurable daily volume targets with automatic pacing
- Gaussian randomization of order sizes to appear natural
- Poisson-distributed trade timing
- multi-wallet support to avoid detection
- integration with exchange APIs on KuCoin, Gate.io, MEXC, and others
- automatic pause when organic volume exceeds a threshold.

**Limitations:**
- Wash trading is illegal or against Terms of Service on most regulated exchanges—this tool should only be used where explicitly permitted (e.g., on certain DEXs or for market-making contracts).
- Excessive or obvious patterns may trigger exchange monitoring systems, leading to account suspension.
- The cost of trading fees accumulates and represents a real expense.
- Creating volume without genuine demand does not create sustainable liquidity—once the bot stops, volume will drop unless organic traders have entered.`
    },
    {
        category: 'trading',
        title: 'Market Making',
...
- inventory skew to maintain delta neutrality
- configurable risk parameters
- support for both CEX (Binance, OKX) and DEX (Uniswap, PancakeSwap) markets
- comprehensive P&L tracking including fees and impermanent loss.

**Limitations:**
- Market making is capital-intensive and carries inventory risk—a strong directional move can leave the system holding a losing position.
- Competition from professional HFT firms means spreads are often too tight for retail market makers to profit.
- On DEXs, impermanent loss can exceed fee income during volatile periods.
- Gas costs on Ethereum mainnet can make frequent position adjustments uneconomical.
- The system requires continuous uptime; any downtime leaves stale quotes that may be adversely filled.`
    },
    {
        category: 'trading',
        title: 'Telegram Copy',
...
**Key Features:**
- Supports 20+ signal message formats from popular providers
- configurable risk per trade (e.g., 1-2% of account)
- automatic position sizing based on stop-loss distance
- multi-TP management with partial closes
- response time under 3 seconds from signal to order
- comprehensive trade log with provider attribution.

**Limitations:**
- Signal quality varies dramatically between providers—the system executes whatever is received, so garbage in = garbage out.
- Slippage can be significant if many users copy the same signal simultaneously.
- Message parsing can fail for unusual formats or typos in signals.
- Telegram rate limits may delay message reception during high traffic.
- The "edge" of signals often diminishes as more people copy them; early subscribers get better fills.`
    },
    {
        category: 'trading',
        title: 'Zerodha EMA',
...
- sector rotation logic to avoid correlated positions
- daily P&L reports via email.

**Limitations:**
- EMA crossover strategies work best in trending markets—they suffer during choppy, range-bound conditions ("whipsaws").
- The Indian market has specific trading hours (9:15 AM - 3:30 PM IST) and no pre/post-market trading, limiting reaction time to overnight news.
- KiteConnect API has rate limits and requires daily login token refresh.
- Brokerage fees and STT (Securities Transaction Tax) can eat into profits on frequent trades.
- The strategy has modest win rates (40-50%) but relies on larger winners than losers.`
    },
    {
        category: 'trading',
...
**Key Features:**
- Real-time scanning of 500+ stocks for extreme moves
- automatic Fibonacci level calculation
- tiered entry at multiple levels for better average price
- strict risk management with predetermined stops
- integration with IBKR's professional-grade execution
- support for both long and short retracement trades.

**Limitations:**
- Mean reversion strategies can fail spectacularly during momentum-driven moves (e.g., earnings surprises, FDA announcements) where price does not retrace.
- The strategy requires precise timing—entering too early catches a "falling knife," too late misses the opportunity.
- IBKR's PDT (Pattern Day Trader) rules require maintaining $25k minimum equity for US accounts.
- The Java TWS API is complex and requires careful error handling for production stability.
...
**Key Features:**
- Automated range rebalancing based on price movement
- volatility-adaptive range width
- multi-pool support across Uniswap (Ethereum) and PancakeSwap (BSC)
- real-time P&L tracking including fees earned vs IL incurred
- gas cost optimization by batching operations
- configurable rebalance thresholds.

**Limitations:**
- Impermanent loss is a fundamental risk in AMM liquidity provision—it cannot be eliminated, only managed.
- High gas fees on Ethereum mainnet can make frequent rebalancing unprofitable; Layer 2 (Arbitrum, Optimism) or BSC are more economical.
- Smart contract risk exists—bugs or exploits in the DEX could result in loss of funds.
- The strategy works best for stablecoin pairs or highly correlated assets where IL is minimized.
- Extreme volatility (flash crashes, rug pulls) can result in significant losses before rebalancing occurs.`
    },
    {
        category: 'trading',
        title: 'Grid Bot',
...
**Key Features:**
- Configurable grid levels (10-200 grids)
- arithmetic or geometric grid spacing
- trailing grid for trend capture
- stop-loss and take-profit for the entire grid
- real-time P&L dashboard built with React
- support for spot and futures trading
- multi-pair grid management.

**Limitations:**
- Grid bots lose money in strong trending markets—if price breaks out above the range, all sell orders fill and the bot holds only the quote currency (missing further upside).
- If price crashes below the range, the bot holds a losing position in the base currency.
- Capital efficiency is low because funds are spread across many grid levels.
- High grid counts increase fees from frequent small trades.
- The strategy requires careful range selection based on support/resistance analysis; a poorly chosen range leads to losses.`
    },
    {
        category: 'trading',
        title: 'Triple EMA',
...
**Key Features:**
- Ultra-low latency Rust implementation
- Triple EMA for reduced lag compared to standard EMA
- ADX filter to avoid trading in choppy conditions
- configurable leverage and position sizing
- automatic websocket reconnection
- comprehensive trade logging with microsecond timestamps.

**Limitations:**
- Scalping is highly sensitive to execution quality—slippage and fees can eliminate thin margins.
- The strategy requires near-perfect uptime; missed signals or delayed execution turns winners into losers.
- High leverage amplifies both gains and losses; a string of losses can quickly blow up an account.
- Binance Futures rate limits may affect order frequency during high-activity periods.
- The strategy works best during volatile market hours (US/EU overlap) and underperforms during low-liquidity Asian sessions.
- Mental fatigue can set in even for automated systems that require monitoring.`
    },
    {
        category: 'trading',
        title: 'CSV Analyzer',
...
**Key Features:**
- Supports tick, minute, hourly, and daily data
...
**Limitations:**
- Backtests are inherently subject to overfitting—a strategy that works on historical data may fail in live markets.
- Data quality is critical; missing data or survivorship bias can skew results.
- The system cannot account for market impact (large orders moving the market) or liquidity constraints.
- Past performance does not guarantee future results.
- Execution in live markets differs from simulated fills.`
    },
    {
        category: 'trading',
        title: 'Renko Charts',
...
**Key Features:**
- Configurable brick size (fixed or ATR-based)
- real-time updates as new price data arrives
- integration with TradingView Lightweight's pan/zoom and crosshair features
- optional wicks to show intra-brick price extremes
- exportable brick data for algorithmic analysis.

**Limitations:**
- Renko charts can obscure the timing of moves—you know price moved X bricks but not exactly when during the day.
- They lose information about volume and intraday volatility.
- Brick size selection is crucial; too small creates noise, too large misses opportunities.
- The library requires manual implementation since it's not a native chart type.
- Not suitable for time-sensitive analysis like news trading.`
    },
    {
        category: 'trading',
        title: 'OCR Signal',
...
**Key Features:**
- Supports multiple annotation styles and chart platforms (TradingView, TrendSpider, etc.)
- automatic price scale detection using axis labels
- color-based zone classification (green = buy, red = sell/stop)
- batch processing for analyzing many screenshots
- integration with Telegram/Discord bots for real-time image analysis.

**Limitations:**
- OCR accuracy depends heavily on image quality; blurry or low-resolution screenshots fail.
- Handwritten annotations are harder to parse than typed text.
- The system may misinterpret decorative elements as trading zones.
- Complex multi-timeframe analyses with many lines can confuse the detector.
- Confirmation by the trader before execution is recommended to avoid costly errors from misreads.`
    },
    {
        category: 'trading',
        title: 'TV Webhook',
...
- configurable position sizing (fixed or percentage)
- secret key authentication to prevent unauthorized orders
- order type selection (market/limit)
- retry logic for failed orders
- comprehensive logging for trade reconciliation
- Telegram notifications for fills and errors.

**Limitations:**
- There is latency between TradingView's alert and order execution (typically 1-5 seconds depending on network); this matters for fast-moving markets.
- TradingView free accounts have limited alert counts.
- The system requires a stable internet connection; downtime means missed signals.
- Ngrok free tier has connection limits and URL changes on restart.
- PineScript strategies may repaint (change signals retroactively), leading to different backtest vs live results.
...
**Key Features:**
- Parallel scanning of 500+ pairs in under 30 seconds
- customizable filter rules with AND/OR logic
- support for 30+ technical indicators
- multi-exchange scanning (Binance, ByBit, OKX, KuCoin)
- historical alert log
- integration with other bots (can auto-trigger trades when conditions match)
- web UI for real-time monitoring.

**Limitations:**
- Scanning introduces slight delays—by the time an alert fires, the opportunity may have partially passed.
- False positives are common; technical conditions don't guarantee profitable trades.
- Data quality from exchange APIs can vary; occasionally erroneous candles trigger false alerts.
- High scan frequency increases API load and may hit rate limits.
- The system requires continuous uptime to not miss setups.
- Condition tuning is an art—too strict finds nothing, too loose creates noise.`
    },
    {
        category: 'trading',
        title: 'Hedge Bot',
...
- automatic hedge activation on drawdown thresholds
- choice of futures (cheaper, unlimited risk) or options (premium cost, capped risk)
- dynamic hedge ratio adjustment
- support for multi-asset portfolios with different betas
- email and Telegram alerts for hedge events.

**Limitations:**
- Hedging costs money—futures carry funding rates, options expire worthless if not needed.
- The system can "whipsaw" if the market drops, triggers a hedge, then immediately recovers (locking in losses).
- Options pricing on Deribit can be illiquid for certain strikes.
- Delta estimation for altcoins vs BTC is imperfect.
- The bot cannot predict crashes—it reacts after drawdown occurs, so the initial loss is not avoided.
- Overhedging can cap upside during recovery.`
    },
    {
        category: 'trading',
        title: 'IBKR Watcher',
...
- configurable alert thresholds for NLV, margin, and position P&L
- multi-channel notifications (SMS, phone call, email, Telegram)
- alert escalation (SMS first, then phone if unacknowledged)
- optional automated defensive actions
- historical alert log and account snapshot storage.

**Limitations:**
- The IBKR Client Portal API requires periodic re-authentication (every 24 hours); if credentials expire, monitoring stops until refreshed.
- TWS connection requires the desktop application to be running.
- There is inherent latency (30-60 seconds) between market moves and alert delivery.
- Twilio has per-message costs.
- Automated position closure during volatile markets can lock in avoidable losses.
...
**Key Features:**
- Terraform-based infrastructure-as-code for version control and reproducibility
- Pub/Sub event-driven triggers for real-time or scheduled actions
- automatic teardown to prevent forgotten resources accruing costs
- multi-environment support (dev/staging/prod)
- cost tracking per project
- Slack notifications for provisioning events.

**Limitations:**
- Terraform state management requires careful attention; state file corruption can cause drift.
- Pub/Sub messages can be delayed during outages, affecting schedule reliability.
- Complex infrastructure graphs have long apply times.
- IAM permissions must be carefully scoped to prevent security issues.
- Debugging failed provisioning across multiple GCP services requires deep platform knowledge.
- Not suitable for truly instant scaling needs—VM spin-up takes 30-60 seconds.`
    },
    {
        category: 'trading',
...
**Key Features:**
- Multi-sheet and multi-tab extraction in a single run
- dynamic column mapping for changing schemas
- data type inference and validation
- null value handling with configurable strategies
- direct database insertion
- Slack/email notifications for job success/failure
- audit logging for data lineage.

**Limitations:**
- Google Sheets API has rate limits (300 requests per minute per project); large sheets may require pagination.
- Sheets with merged cells or irregular structures fail the parser.
- Formula-dependent cells return computed values, not formulas.
- Changes to sheet structure (renamed columns) can break extraction until mappings are updated.
- OAuth2 token refresh requires periodic re-authorization for certain grant types.`
    },
    {
        category: 'trading',
...
**Key Features:**
- Headless browser for full JavaScript rendering
- login flow automation with session persistence
- infinite scroll and pagination handling
- proxy rotation and user-agent randomization
- data export to CSV, JSON, or databases
- retry logic for failed page loads
- screenshot capture for debugging.

**Limitations:**
- Selenium is slow compared to HTTP-based scraping—each page load takes 3-10 seconds.
- ChromeDriver must match the installed Chrome version; updates can break the setup.
- Heavily protected sites (Cloudflare, reCAPTCHA) may still block access.
- Scraping may violate website Terms of Service and could have legal implications.
- Resource-intensive—each browser instance uses significant RAM.
- Not suitable for scraping thousands of pages quickly.`
    },
    {
        category: 'trading',
...
**Key Features:**
- Custom spreadsheet functions callable from any cell
- automated validation and formatting on data entry
- scheduled report generation and email distribution
- integration with external APIs (fetch live data, send notifications)
- menu-based UI for non-technical users
- logging and audit trail for changes.

**Limitations:**
- Google Apps Script has a 6-minute execution limit for regular scripts (30 minutes for some Google Workspace tiers).
- API quotas apply (e.g., UrlFetch: 20,000 calls/day).
- Debugging is less robust than full IDE environments.
- Complex logic in Scripts makes spreadsheets slow and harder to maintain.
- No version control integration; code lives in the script editor.
- Breaking API changes or deprecated methods can silently fail.`
    },
    {
        category: 'trading',
...
**Key Features:**
- Sheet-by-sheet comparison with row/column alignment handling
- color-coded difference highlighting in output file
- summary report of all changes
- formula vs value comparison toggle
- handling of inserted/deleted rows (not just changed values)
- PDF summary export for auditors.

**Limitations:**
- Row insertion/deletion detection relies on heuristics and can produce false positives for heavily restructured sheets.
- Large files with 1M+ cells can still be slow (minutes to process).
- Formatting differences (fonts, borders) are not compared—only data.
- Password-protected sheets must be unlocked first.
- The tool does not track change history; it only compares two snapshots.
- Images and charts embedded in Excel are not compared.`
    },
...
**Key Features:**
- Support for multiple document types (PDF, DOCX, TXT, HTML)
- automatic chunking with overlap for context continuity
- hybrid search (keyword + semantic) for better recall
- conversation memory for follow-up questions
- citation of source document and page number
- web UI with streaming responses.

**Limitations:**
- RAG accuracy depends on document quality—outdated or contradictory documents lead to wrong answers.
- Retrieval can fail for questions requiring synthesis across many documents.
- Embedding models have limits (~8k tokens per call).
- Large document sets require significant vector database storage.
- Latency increases with retrieval complexity.
- The system cannot reason beyond what's in the documents—it won't "know" things not indexed.`
    },
    {
        category: 'ai',
        title: 'Hand Gesture',
...
- support for 10+ customizable gestures
- MediaPipe for robust hand detection under varying lighting
- configurable gesture-to-action mappings
- works with any application (sends standard keyboard commands)
- training mode to add new custom gestures.

**Limitations:**
- Accuracy drops in poor lighting or when hands are partially occluded.
- Two-handed gestures are harder to distinguish.
- Background clutter with skin-tone colors can confuse detection.
- Latency of ~100-200ms may feel slow for gaming.
- The model was trained on adult hands—children's smaller hands may have reduced accuracy.
- Requires a clear view of the hand; gestures under a table or off-camera edges won't work.`
    },
    {
        category: 'ai',
...
**Key Features:**
- Multi-stage enhancement pipeline combining classical CV and deep learning
- U-Net for precise fingerprint region segmentation
- Gabor filter banks for ridge enhancement
- Super-resolution GAN for reconstructing degraded areas
- batch processing for multiple images
- export compatible with AFIS systems.

**Limitations:**
- The GAN can "hallucinate" ridge patterns that don't exist in the original—potentially creating false minutiae.
- Extremely damaged prints (over 70% missing) cannot be reliably enhanced.
- The model was trained primarily on optical sensor prints; latent prints from crime scenes (which may have different textures) require fine-tuning.
- Forensic use requires careful validation; enhanced prints should be treated as leads, not evidence.
- GPU recommended for GAN inference speed.`
    },
    {
        category: 'ai',
        title: 'MCP Server',
...
**Key Features:**
- Full implementation of MCP resource and tool specifications
- file system, database (PostgreSQL, SQLite), and API resource types
- tool execution with sandboxed permissions
- API key and OAuth2 authentication
- schema discovery for dynamic integration
- logging of all AI interactions for audit.

**Limitations:**
- MCP is a new, evolving specification—breaking changes may occur.
- Not all AI clients support MCP natively (may require plugins or adapters).
- Tool execution poses security risks if permissions are too broad; careful scoping is essential.
- Performance depends on the underlying data source—slow database queries mean slow AI responses.
- The server must run continuously for AI access; downtime breaks integrations.
- No built-in caching—repeated queries hit the data source each time.`
    },
    {
        category: 'ai',
...
- probability calibration for trading thresholds
- model explainability via attention weights.

**Limitations:**
- Financial markets are inherently unpredictable; even 60% directional accuracy is considered good.
- Overfitting is a severe risk—models can memorize spurious patterns.
- Performance degrades during regime changes (bull to bear market).
- High-frequency prediction (minute bars) is much harder than daily/weekly.
- Latency and execution costs are not modeled.
- The model cannot predict black swan events (sudden news, crashes).
- Profitable backtests often fail in live trading due to slippage, fees, and market impact.`
    },
    {
        category: 'ai',
        title: 'CNN Patterns',
...
**Key Features:**
- Recognition of 20+ candlestick patterns
- transfer learning from VGG16 for faster training
- data augmentation for robustness
- real-time inference pipeline (capture → classify → alert)
- confidence thresholds to reduce false positives
- integration with trading systems for automated entries.

**Limitations:**
- Pattern recognition does not imply profitable trading—many patterns fail to predict the expected move.
- Screenshot quality affects accuracy; low-resolution or cluttered charts fail.
- The model was trained on a specific charting style; different platforms (light mode, different candle colors) require retraining.
- Class imbalance (rare patterns) can skew predictions.
- Visual pattern recognition inherently loses the numerical precision of raw data.
- Some patterns are subjective even for humans, introducing label noise.`
    },
    {
        category: 'ai',
        title: 'GAN Generator',
...
- style mixing for creative variations
- latent space interpolation for morphing animations
- truncation control for diversity
- trained model checkpoint for instant generation
- web interface for non-technical users.

**Limitations:**
- Training requires expensive GPU hardware (A100 recommended, V100 minimum).
- Generated images inherit biases from the training dataset.
- The model cannot generate specific requested content—only random samples in the learned style.
- Copyright status of AI-generated art is legally ambiguous.
- Outputs can occasionally include artifacts or deformities.
- Fine-tuning on new styles requires retraining or transfer learning.`
    },
    {
        category: 'ai',
...
**Key Features:**
- Visual demonstration of neuroevolution
- real-time multi-agent gameplay display
- configurable NEAT parameters (population size, mutation rates)
- fitness graphs showing progress across generations
- ability to save/load evolved champions
- educational codebase for learning NEAT.

**Limitations:**
- NEAT is computationally expensive for complex environments—Flappy Bird is ideal due to its simplicity.
- Evolution can get stuck in local optima (e.g., birds that only dodge low pipes).
- Results vary between runs due to stochastic nature.
- Not applicable to games requiring long-term planning or complex state.
- The learned behavior is specific to the environment—different pipe physics require retraining.`
    },
    {
        category: 'ai',
...
- support for LLMs (GPT, Claude, Llama) and diffusion models
- automatic CLIP/perplexity scoring
- human preference collection interface
- W&B integration for experiment tracking
- early stopping for cost efficiency
- exportable optimal config.

**Limitations:**
- Optimization requires many API calls—can be expensive for commercial APIs (GPT-4).
- Quality metrics (CLIP score) don't always correlate with human preference.
- Different prompts may have different optimal settings; results may not generalize.
- Human labeling for preference data is time-consuming.
- The search space grows exponentially with parameters—focus on 3-5 key hyperparameters.`
    },
    {
        category: 'ai',
        title: 'PPO Predictor',
...
- PPO with custom reward shaping for Sharpe optimization
- support for multiple assets
- walk-forward backtesting
- TensorBoard logging of training progress
- live inference mode for signal generation.

**Limitations:**
- Reinforcement learning for trading is notoriously difficult—most agents overfit to training data and fail on new data.
- Reward design is crucial and hard to get right.
- Transaction costs and slippage are approximated in simulation but differ in reality.
- Training is computationally intensive and unstable (requires many runs).
- The agent cannot adapt to regime changes without retraining.
- Live deployment requires robust risk management around the agent's signals.`
    },
    {
        category: 'ai',
        title: 'Medicine Scan',
...
**Key Features:**
- Dual OCR + visual classification pipeline
- trained on NIH Pill Image Dataset (10,000+ pill images)
- drug interaction checking via API
- works on mobile (TensorFlow Lite) and desktop
- confidence scores for verification
- support for multiple pills in one image.

**Limitations:**
- Accuracy is limited for generic pills without imprints or with worn markings.
- Similar-looking pills from different manufacturers may be confused.
- The model was trained on US medications; international drugs may fail.
- Blurry or poorly lit photos reduce accuracy.
- The system provides identification, not medical advice—users should always confirm with pharmacists.
- Pill databases require periodic updates as new drugs are approved.`
    },
    {
        category: 'ai',
        title: 'CNN BMI',
...
- BMI category classification with ranges
- gamified health awareness UI
- privacy-preserving (no images sent to servers).

**Limitations:**
- This is NOT a medical tool—predictions have high error rates (typically ±5 BMI units).
- Facial features are poor predictors of body composition compared to weight/height measurement.
- The training data relies on self-reported BMI, introducing label noise.
- Accuracy varies by age, ethnicity, and gender (model trained primarily on adults).
- Face angle, lighting, and expressions affect results.
- Should never be used for medical decisions.
...
**Key Features:**
- RS485 differential signaling for industrial-grade reliability
- Modbus RTU protocol for standardized communication
- sub-10ms latency for instant response
- Home Assistant integration for smart home features
- support for 127 nodes per bus
- fail-safe modes for critical devices.

**Limitations:**
- Wired installation is labor-intensive compared to WiFi retrofits—best for new construction or renovation.
- RS485 requires proper cable runs (twisted pair, preferably shielded).
- Modbus polling introduces slight latency as more devices are added.
- Firmware updates require physical access or OTA per-node.
- Debugging bus issues (termination, grounding) can be challenging.
- Limited to on/off and dimming—complex devices may need custom protocol implementation.`
    },
    {
        category: 'iot',
        title: 'Water Level',
...
**Key Features:**
- Non-contact ultrasonic measurement (no corrosion issues)
- WiFi connectivity for remote monitoring
- Node-RED dashboard with real-time gauge
- configurable high/low alert thresholds
- deep sleep mode for battery operation
- historical data logging.

**Limitations:**
- Ultrasonic sensors can give false readings if condensation forms on the sensor face.
- Narrow tank openings cause echo interference.
- WiFi range limits outdoor tank placement; a repeater or cellular module may be needed.
- The HC-SR04 is not rated for outdoor use without enclosure.
- Battery-powered versions need solar panels or periodic recharging.
- Accuracy is ±2cm, which may not suffice for small tanks.`
    },
    {
        category: 'iot',
        title: 'Fish Feeder',
...
**Key Features:**
- Scheduled feeding via RTC with battery backup
- manual remote feeding via Blynk app
- adjustable portion sizes
- last-feed timestamp display
- vacation mode (increased portions)
- low-food warning (optional sensor).

**Limitations:**
- The dispenser mechanism can jam with wet or clumped food; dry pellets or flakes are recommended.
- WiFi connectivity is required for remote features—RTC handles local scheduling offline.
- The servo can wear over time; periodic maintenance is needed.
- Overfeeding due to multiple manual triggers is possible—consider a cooldown period.
- Power outages with depleted RTC battery will reset the schedule.
- Not suitable for live food or frozen food dispensing.`
    },
    {
        category: 'iot',
        title: 'Ghost Lamp',
...
**Key Features:**
- 3-axis articulated motion (pan, tilt, nod)
- sound-responsive "listening" behavior
- gesture control for brightness and power
- RGB LED with color temperature adjustment
- idle animations for personality
- configurable behavior profiles.

**Limitations:**
- Servo noise can be audible in quiet environments.
- The sound-direction detection works best in quiet rooms; background noise confuses it.
- The mechanical structure requires precise 3D printing or assembly to avoid binding.
- Servos draw significant current—a dedicated 5V power supply is needed.
- Repeated rapid movements can wear servo gears.
- The lamp is a curiosity piece, not practical task lighting due to its motion focus.`
    },
    {
        category: 'iot',
        title: 'Fan Control',
...
**Key Features:**
- PID control for smooth, stable temperature regulation
- TRIAC dimming for continuous speed control (not stepped)
- WiFi connectivity for smart home integration
- voice control via Alexa/Google Home
- manual override button
- temperature logging for analysis.

**Limitations:**
- TRIAC dimming works for AC induction fans but not DC or BLDC fans (which require different control).
- Some fans hum or buzz at low speeds due to TRIAC waveform distortion.
- High-wattage fans may require a beefier TRIAC module.
- The DHT22 sensor has ±0.5°C accuracy; placement matters (avoid drafts or heat sources).
- PID tuning requires experimentation for optimal response.
- Power cuts reset the ESP32; last settings are restored from flash memory.
- Safety: working with mains AC voltage requires proper electrical knowledge.`
    }
];
...
function createProjectCard(project) {
    ...
    // Extract tech string or fallback, and split by comma to create an array for bullets
    const techString = project.tech || "Python, Automated Systems, Details Pending";
    const techArray = techString.split(',').map(tech => tech.trim()).filter(t => t);

    // Create bullet point HTML
    const techBullets = `<ul class="tech-list">${techArray.map(tech => `<li>${tech}</li>`).join('')}</ul>`;

    // Use goal for longer description, truncate if too long
    let mainDesc = project.goal || project.desc;
    if (mainDesc && mainDesc.length > 110) {
        mainDesc = mainDesc.substring(0, 107) + "...";
    }
        ...
        <div class="card-inner">
            <div class="card-front">
                <div class="card-front-content">
                    ...
                    <p class="card-desc">${mainDesc}</p>
                </div>
            </div>
            <div class="card-back">
                <p><strong>Goal:</strong><br>${project.goal || 'See details for full scope.'}</p>
                ${tagsHtml}
...
function openSectorOverlay(categoryName) {
    ...
    // Add pushState for browser back button
    history.pushState({ overlay: 'sector', category: categoryName }, '', `#projects-${categoryName}`);
...
function closeSectorOverlay(isPopState = false) {
    const overlay = document.getElementById('sector-overlay');
    if (!overlay.classList.contains('active')) return;

    overlay.classList.remove('active');

    // Remove state if called manually (not via browser back)
    if (!isPopState && history.state && history.state.overlay === 'sector') {
        history.back();
    }
...
function openProjectDetails(project) {
    ...
    // Add pushState for browser back button
    history.pushState({ overlay: 'details', title: project.title }, '', `#details-${project.title.replace(/\s+/g, '-').toLowerCase()}`);
    ...
    // Helper function to render text blocks, supporting bullet points
    function renderParagraphs(text, fallback) {
        ...
        lines.forEach(line => {
            ...
            // Check if line starts with a bullet point character (- or *)
    ...
    detailsOverlay.querySelector('.details-overview').innerHTML = renderParagraphs(overviewText, 'No overview available.');
    detailsOverlay.querySelector('.details-how-it-works').innerHTML = renderParagraphs(howItWorksText, 'No details available.');
    detailsOverlay.querySelector('.details-key-features').innerHTML = renderParagraphs(keyFeaturesText, 'No features listed.');
    detailsOverlay.querySelector('.details-limitations').innerHTML = renderParagraphs(limitationsText, 'No limitations noted.');
...
function closeProjectDetails(isPopState = false) {
    const detailsOverlay = document.getElementById('project-details-overlay');
    if (!detailsOverlay.classList.contains('active')) return;

    detailsOverlay.classList.remove('active');

    // Remove state if called manually
    if (!isPopState && history.state && history.state.overlay === 'details') {
        history.back();
    }

    const sectorOverlay = document.getElementById('sector-overlay');
    const isGridActive = sectorOverlay && sectorOverlay.classList.contains('active');

    if (!isGridActive) {
        // Restore UI elements when closing project details only if not returning to grid
        const navbar = document.getElementById('navbar');
...
window.addEventListener('popstate', (e) => {
    // If the state implies we are going BACK from a details page
    if (document.getElementById('project-details-overlay').classList.contains('active')) {
        closeProjectDetails(true);
        // If we also need to close the sector overlay (e.g. going way back), let the next popstate handle it
    }
    // If we are going BACK from a grid page
    else if (document.getElementById('sector-overlay').classList.contains('active')) {
        closeSectorOverlay(true);
    }
});
...
if (contactSendBtn) {
    contactSendBtn.addEventListener('click', async () => {
        ...
        const btnOriginalText = contactSendBtn.textContent;
